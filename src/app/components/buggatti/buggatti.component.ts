...
@Component({
  selector: 'app-buggatti',
  templateUrl: './buggatti.component.html',
  styleUrls: ['./buggatti.component.css']
})
export class BuggattiComponent implements OnInit {
  ...
  this.fromDateMin= Year +'-' + Month +'-' + date;
   ...
   this.data.GetAllCars().subscribe(data => {
    this.cars = data.filter((car:any)=> car.Brand=='Buggatti');
   })
  }
